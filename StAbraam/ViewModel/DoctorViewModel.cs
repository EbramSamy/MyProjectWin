...
{
    class DoctorViewModel : ViewModelBase, IDataErrorInfo
    {
        DataProvider _data;
        ...
        {
            get { return this._currentDoctor; }
            set
            {
                if (Equals(value, _currentDoctor))
                {
                    return;
                }
                _currentDoctor = value;
                RaisePropertyChanged("CurrentDoctor");
            }
        }
        ...
        private string _searchNamel = "";
        public string SearchName
        {
            get { return this._searchNamel; }
            set
            {
                if (Equals(value, _searchNamel))
                {
                    return;
                }
                _searchNamel = value;
                RaisePropertyChanged("SearchNamel");
            }
        }

        private string _searchNationalID = "";
        public string SearchNationalID
        {
            get { return this._searchNationalID; }
            set
            {
                if (Equals(value, _searchNationalID))
                {
                    return;
                }
                _searchNationalID = value;
                RaisePropertyChanged("SearchNationalID");
            }
        }
        ...
        private ICommand saveDoctorCommand;
        public ICommand SaveDoctorCommand
        {
            get
            {
                return saveDoctorCommand ?? (saveDoctorCommand = new RelayCommand(() =>
                {
                    _data.UpdateDoctor(CurrentDoctor, FullName, NationalID, DateOfBirth, Mobiles, Email, SelectedCountry, SelectedCity, Diocese, SelectedSpecialty, SelectedSubSpecialty, CurrentOccupation, SpokenLanguages, CurrentAffiliation, Clinics, ServedList, ToJoinList, Periods, Suggestions);
                    Messenger.Default.Send<string>(Utils.DoctorAddedMessage);
                }));
            }
            set
            {
                saveDoctorCommand = value;
            }
        }
        ...
        public ICommand GoCommand
        {
            get
            {
                return _goCommand ?? (_goCommand = new RelayCommand(() =>
                {
                    ...
                    CurrentDoctor = _data.SearchForDoctor(SearchName, SearchNationalID).FirstOrDefault();
                    if (CurrentDoctor != null)
                    {
                        ...
                        Mobiles = new ObservableCollection<Mobile>(CurrentDoctor.Mobiles);
                        SelectedCountry = CurrentDoctor.Doctor_Countries.Select(c => c.Country).ToList().FirstOrDefault();
                        SelectedCity = CurrentDoctor.Doctor_Cities.Select(c => c.City).ToList().FirstOrDefault();
                        Diocese = CurrentDoctor.Diocese;
                        SelectedSpecialty = CurrentDoctor.Doctor_Specialty.Select(c => c.Specialty).ToList().FirstOrDefault();
                        SelectedSubSpecialty = CurrentDoctor.Doctor_Sub_Specialty.Select(c => c.Sub_Specialty).ToList().FirstOrDefault();
                        CurrentOccupation = CurrentDoctor.CurrentOccupation;
                        SpokenLanguages = new ObservableCollection<Language>(CurrentDoctor.Doctor_Languages.Select(l => l.Language).ToList());
                        CurrentAffiliation = CurrentDoctor.CurrentHospital;
                        Clinics = new ObservableCollection<Clinic>(CurrentDoctor.Doctor_Clinic.Select(c => c.Clinic).ToList());
                        ServedList = new ObservableCollection<Service>(CurrentDoctor.Doctor_CurrentService.Select(s => s.Service).ToList());
                        ToJoinList = new ObservableCollection<Service>(CurrentDoctor.Doctor_ToJoinServices.Select(s => s.Service).ToList());
                        Periods = new ObservableCollection<ServiceAbroadPeriod>(CurrentDoctor.Doctor_PreferedPeriod.Select(s => new ServiceAbroadPeriod(s.Period)).ToList());
                    }
